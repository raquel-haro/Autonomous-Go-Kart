...
#define ARRAY_END 1439
#define CAR_WIDTH 64
#define CAR_LENGTH 137
...
{
	float movingAverage[241];            // 241 because 0 to 240 is 241 points
    ...
    int numElementsInBin = 20;          //data points held in each pin
    ...
    //ROS_INFO("Straight Ahead: %f", scan->ranges[0]); //ranges array has 1440 points indexed 0-1439
	...
	for(int i=-120; i<=120; i++){
		...
		movingAverage[i+120] = temp; // offset by 120 because the loop starts at i=-120
        
        /*if(i<0) {
            placeholder = scan->ranges[1439+i];
        } else {
            placeholder = scan->ranges[i];
        }
        ROS_INFO("MovingAverage: %f, Not Averaged: %f", movingAverage[i+120], placeholder);*/
	} 
...
//======================================   BINNING STARTS HERE   ==========================================
    ...
    for(int i = 0; i<240; i++) {
        for(int bin=0; bin<numAngles-1; bin++){                 //check element against each bin
		    if((i-120)>angles[bin] && (i-120)<=angles[bin+1]){	// if element fits in current bin
			    temp1=0;
	...
	for(int bin=0; bin<numAngles-1; bin++){
		...
		if( ((sumBinElements/numValidPoints)>largestAverage) && (numValidPoints>0) ){
            ...
            if(internalAngle==2.5 || internalAngle==-2.5) {
                internalAngle = 0;
            }
        }
    }
